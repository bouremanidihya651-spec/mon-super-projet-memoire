...
const HotelDetailPage = ({ hotel, onBack, openAuthModal }) => {
  ...
  const fetchReviews = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_URL}/api/reviews/hotel/${hotel.id}`);
  ...
  const checkFavoriteStatus = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/favorites/check/hotel/${hotel.id}`,
    ...
    >
      ...
      >
        ...
        {hotel.price_per_night && (
          <div className="absolute bottom-4 right-4 px-4 py-2 bg-[#2d7a5a] rounded-full border border-[#e0dcd4] shadow-lg">
            <span className="text-white font-bold text-lg">{hotel.price_per_night} DA/nuit</span>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-5 flex-wrap py-5 border-t border-b border-[#e0dcd4] mb-7">
        ...
        {hotel.city && (
          <div className="flex items-center gap-2 text-[#6b8f7b]">
            <MapPin className="w-5 h-5 text-[#2d7a5a]" />
      ...
      {hotel.description && (
        <div className="mb-7">
          ...
          <p className="text-[#6b8f7b] leading-relaxed">{hotel.description}</p>
        </div>
      )}
      ...
      {hotel.amenities && hotel.amenities.length > 0 && (
        <div className="mb-7">
          <h3 className="text-lg font-bold mb-3 flex items-center gap-2 text-[#1a4a36]">
            <span className="text-[#2d7a5a]"><Sparkles size={16} /></span>
          ...
          <div className="flex flex-wrap gap-3">
            {hotel.amenities.map((amenity, index) => {
              ...
              return (
                <div key={index} className="flex items-center gap-2 bg-[#f7f5f0] px-4 py-2 rounded-full border border-[#e0dcd4]">
                  <Icon className="w-4 h-4 text-[#2d7a5a]" />
                  <span className="text-sm text-[#1a4a36]">{amenity}</span>
                </div>
              );
            })}
          </div>
      ...
      {/* Contact Info */}
      {(hotel.phone || hotel.website || hotel.address) && (
        <div className="mb-7">
          <h3 className="text-lg font-bold mb-3 flex items-center gap-2 text-[#1a4a36]">
            <span className="text-[#2d7a5a]"><Phone size={16} /></span>
            Contact
          </h3>
          <div className="space-y-2">
            ...
            {hotel.phone && (
              <div className="flex items-center gap-2 text-[#6b8f7b]">
                <Phone className="w-4 h-4 text-[#2d7a5a]" />
                <span className="text-sm">{hotel.phone}</span>
              </div>
            )}
            {hotel.website && (
              <div className="flex items-center gap-2 text-[#6b8f7b]">
                <Globe className="w-4 h-4 text-[#2d7a5a]" />
                <a href={hotel.website} target="_blank" rel="noopener noreferrer" className="text-sm text-[#2d7a5a] hover:underline">
                  {hotel.website}
                </a>
              </div>
            )}
