...
const ReservationDetailsModal = ({ reservation, onClose }) => {
            ...
            <div className="space-y-3">
              ...
              {reservation.transport?.departure_city && reservation.transport?.arrival_city && (
                <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
                  ...
                  <span className="font-medium text-[#1a4a36] dark:text-dark-text">{reservation.transport.departure_city} → {reservation.transport.arrival_city}</span>
                </div>
              )}
            ...
            <div className="space-y-3">
              ...
              {reservation.hotel?.stars && (
                <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
                  ...
                  <span className="font-medium text-[#2d7a5a] dark:text-surface">{'★'.repeat(reservation.hotel.stars)} ({reservation.hotel.stars} étoiles)</span>
                </div>
              )}
              ...
              <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
                ...
                <span className="font-medium text-[#1a4a36] dark:text-dark-text">{new Date(reservation.departure_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
                <span className="text-[#6b8f7b] dark:text-dark-text-muted">Check-out</span>
                <span className="font-medium text-[#1a4a36] dark:text-dark-text">{new Date(reservation.return_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
              </div>
              ...
              {reservation.hotel?.amenities && (
                <div className="py-3 border-b border-[#e0dcd4] dark:border-dark-border">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(() => {
                      try {
                        const amenities = typeof reservation.hotel.amenities === 'string' ? JSON.parse(reservation.hotel.amenities) : reservation.hotel.amenities;
                        return amenities?.map((amenity, idx) => (
                          <span key={idx} className="px-2 py-1 bg-[#2d7a5a]/10 dark:bg-dark-bg text-[#2d7a5a] dark:text-surface text-xs rounded border border-[#e0dcd4] dark:border-dark-border">{amenity}</span>
                        ));
                      } catch { return null; }
                    })()}
                  </div>
          ...
          <div className="border-t border-[#e0dcd4] dark:border-dark-border pt-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
                ...
                <span className="font-medium">{new Date(reservation.departure_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
              </div>
              ...
              <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
                ...
                <span className="font-medium">{new Date(reservation.return_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
              </div>
                ...
                <div className="py-3 border-b border-[#e0dcd4] dark:border-dark-border">
                  ...
                  {reservation.travelers_details[0].firstName && (
                    <div className="flex justify-between py-1">
                      ...
                      <span className="font-medium text-sm">{reservation.travelers_details[0].firstName} {reservation.travelers_details[0].lastName}</span>
                    </div>
                  )}
          ...
          <div className="border-t border-[#e0dcd4] dark:border-dark-border pt-6">
            ...
            <div className="space-y-3">
              ...
              {reservation.transport?.departure_city && reservation.transport?.arrival_city && (
                <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
                  <span className="text-[#6b8f7b] dark:text-dark-text-muted">Trajet</span>
                  <span className="font-medium">{reservation.transport.departure_city} → {reservation.transport.arrival_city}</span>
                </div>
              )}
              {reservation.pickup_time && (
                <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
          ...
          <div className="border-t border-[#e0dcd4] dark:border-dark-border pt-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
                ...
                <span className="font-medium">{new Date(reservation.departure_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', weekday: 'long' })}</span>
              </div>
              {reservation.pickup_time && (
                <div className="flex justify-between py-2 border-b border-[#e0dcd4] dark:border-dark-border">
              ...
              {reservation.activity?.tags && (
                <div className="py-3 border-b border-[#e0dcd4] dark:border-dark-border">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(() => {
                      try {
                        const tags = typeof reservation.activity.tags === 'string' ? JSON.parse(reservation.activity.tags) : reservation.activity.tags;
                        return tags?.map((tag, idx) => (
                          <span key={idx} className="px-2 py-1 bg-[#2d7a5a]/10 text-[#2d7a5a] text-xs rounded border border-[#e0dcd4] dark:border-dark-border">{tag}</span>
                        ));
                      } catch { return null; }
                    })()}
                  </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#f7f5f0] dark:bg-dark-bg rounded-3xl border border-[#e0dcd4] dark:border-dark-border max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="text-2xl font-bold text-[#2d7a5a]">{parseFloat(reservation.total_price || 0).toFixed(2)} DA</div>
            </div>
          </div>
          ...
          <div className="border-t border-[#e0dcd4] dark:border-dark-border pt-6">
            <p className="text-xs text-[#6b8f7b] dark:text-dark-text-muted">
              Réservation créée le {new Date(reservation.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
            </p>
          </div>
        </div>

        <div className="sticky bottom-0 bg-[#f7f5f0] dark:bg-dark-bg border-t border-[#e0dcd4] dark:border-dark-border p-6 flex gap-3">
          <button onClick={onClose} className="flex-1 px-6 py-3 bg-[#f7f5f0] dark:bg-dark-bg hover:bg-[#f7f5f0] dark:bg-dark-bg border border-[#e0dcd4] dark:border-dark-border rounded-full font-semibold transition-all">
            Fermer
          </button>
          {reservation.payment_status === 'paid' && (
            <button
              onClick={() => { window.open(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/invoices/${reservation.id}/download`, '_blank'); }}
    ...
    <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <div className={`text-2xl font-bold ${filterCategory === key ? 'text-[#2d7a5a]' : 'text-[#1a4a36] dark:text-dark-text'}`}>{count}</div>
              </div>
              <div className={`text-xs ${filterCategory === key ? 'text-[#2d7a5a]' : 'text-[#6b8f7b] dark:text-dark-text-muted'}`}>{config.label}</div>
            </button>
          );
        })}
      </div>
      ...
      {filterCategory !== 'all' && (
        <div className="mb-6 flex items-center gap-2">
          ...
          <span className="px-3 py-1 bg-[#2d7a5a]/10 text-[#2d7a5a] rounded-full text-sm font-medium border border-[#e0dcd4] dark:border-dark-border">{categoryConfig[filterCategory]?.label}</span>
          <button onClick={() => setFilterCategory('all')} className="text-[#6b8f7b] dark:text-dark-text-muted hover:text-[#1a4a36] dark:text-dark-text text-sm underline">Effacer</button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <button onClick={fetchReservations} className="px-6 py-2 bg-[#2d7a5a] text-black rounded-full font-semibold hover:bg-[#1a4a36] transition-all">Réessayer</button>
        </div>
        ...
        <div className="text-center py-20">
          ...
          <p className="text-[#6b8f7b] dark:text-dark-text-muted text-sm">{filterCategory === 'all' && filterStatus === 'all' && "Réservez votre premier voyage pour commencer"}</p>
        </div>
      ) : (
        <div>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-[#1a4a36] dark:text-dark-text">
              {filterCategory === 'all' ? 'Toutes les réservations' : categoryConfig[filterCategory]?.label}
              <span className="text-[#6b8f7b] dark:text-dark-text-muted text-sm font-normal ml-3">({filteredReservations.length} {filteredReservations.length > 1 ? 'réservations' : 'réservation'})</span>
            </h2>
          </div>
