...
const FlightReservationModal = ({ isOpen, onClose, transport, user }) => {
  ...
  const formatAirportName = (airport) => { if (!airport) return 'N/A'; return airport; };
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
        ...
        >
          ...
          <div className="text-center">
            ...
            <button onClick={onClose} className="w-full bg-[#2d7a5a] text-black font-bold py-3 rounded-full hover:bg-[#1a4a36] transition">
              Retour à l'accueil
            </button>
          </div>
        </motion.div>
      ...
      >
            ...
            <React.Fragment key={s}>
              <div className={`flex items-center gap-2 ${step >= s ? 'text-[#2d7a5a]' : ''}`} style={{ color: step < s ? t.textMuted : '' }}>
                <div
                  ...
                  style={{ backgroundColor: step >= s ? t.primary : t.bgBadge, color: step >= s ? '#000' : t.textMuted }}
                >
                  {s}
                </div>
              ...
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setTripType('round_trip')} className="flex-1 py-3 px-4 rounded-full font-medium transition"
                      style={{ backgroundColor: tripType === 'round_trip' ? t.primary : t.bgBadge, color: tripType === 'round_trip' ? '#000' : t.textMuted }}>
                      Aller-retour
                    </button>
                    <button onClick={() => setTripType('one_way')} className="flex-1 py-3 px-4 rounded-full font-medium transition"
                      style={{ backgroundColor: tripType === 'one_way' ? t.primary : t.bgBadge, color: tripType === 'one_way' ? '#000' : t.textMuted }}>
                      Aller simple
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label style={{ color: t.text }} className="font-medium mb-2 block">
                      <Calendar className="w-4 h-4 inline mr-1" /> Départ
                    </label>
                  ...
                  {tripType === 'round_trip' && (
                    <div>
                      <label style={{ color: t.text }} className="font-medium mb-2 block">
                        <Calendar className="w-4 h-4 inline mr-1" /> Retour
                      </label>
                ...
                <div>
                  <label style={{ color: t.text }} className="font-medium mb-3 block">
                    <Users className="w-4 h-4 inline mr-1" /> Voyageurs
                  </label>
                  <div style={{ backgroundColor: t.bgInput, borderColor: t.border }} className="border rounded-3xl p-6 space-y-4">
                    {[['adults', 'Adultes', '(12 ans et +)', 1], ['children', 'Enfants', '(2-11 ans)', 0], ['infants', 'Bébés', '(moins de 2 ans)', 0]].map(([key, label, desc, min]) => (
                      <div key={key} className="flex items-center justify-between">
                        <div>
                          <p style={{ color: t.text }} className="font-medium">{label}</p>
                          <p style={{ color: t.textMuted }} className="text-xs">{desc}</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <button onClick={() => setTravelers(prev => ({ ...prev, [key]: Math.max(min, prev[key] - 1) }))}
                            style={{ backgroundColor: t.bgBadge, color: t.text }} className="w-10 h-10 rounded-full hover:opacity-70 transition">-</button>
                          <span style={{ color: t.text }} className="font-bold w-8 text-center">{travelers[key]}</span>
                          <button onClick={() => setTravelers(prev => ({ ...prev, [key]: prev[key] + 1 }))}
                            style={{ backgroundColor: t.bgBadge, color: t.text }} className="w-10 h-10 rounded-full hover:opacity-70 transition">+</button>
                        </div>
                      </div>
                    ))}
                    <div style={{ borderColor: t.border }} className="pt-4 border-t">
                      <button onClick={handleNextStep} className="w-full bg-[#2d7a5a] text-black font-bold py-3 rounded-full hover:bg-[#1a4a36] transition flex items-center justify-center gap-2">
                        Appliquer | {totalTravelers} Voyageur(s) <ChevronRight className="w-5 h-5" />
                      </button>
                    </div>
                  </div>
                </div>
                ...
                {travelerDetails.map((traveler, index) => (
                  <div key={index} style={{ backgroundColor: isDark ? '#2d7a5a0d' : '#f7f5f080', borderColor: t.border }} className="border rounded-3xl p-6">
                    <div className="flex items-center gap-3 mb-4">
                      <div style={{ backgroundColor: '#2d7a5a33', color: t.primary }} className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">{index + 1}</div>
                      <h4 style={{ color: t.text }} className="font-medium">Voyageur {index + 1} {index === 0 && '(Principal)'}</h4>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="col-span-2 sm:col-span-1">
                        <label style={{ color: t.textMuted }} className="text-xs mb-1 block">Prénom *</label>
                        <input type="text" value={traveler.firstName} onChange={(e) => updateTravelerDetail(index, 'firstName', e.target.value)}
                          style={{ backgroundColor: t.bgBadge, borderColor: t.border, color: t.text }}
                          className="w-full border rounded-full px-4 py-2 text-sm focus:border-[#2d7a5a] outline-none" placeholder="Prénom" />
                      </div>
                      <div className="col-span-2 sm:col-span-1">
                        <label style={{ color: t.textMuted }} className="text-xs mb-1 block">Nom *</label>
                        <input type="text" value={traveler.lastName} onChange={(e) => updateTravelerDetail(index, 'lastName', e.target.value)}
                          style={{ backgroundColor: t.bgBadge, borderColor: t.border, color: t.text }}
                          className="w-full border rounded-full px-4 py-2 text-sm focus:border-[#2d7a5a] outline-none" placeholder="Nom" />
                      </div>
                      <div>
                        <label style={{ color: t.textMuted }} className="text-xs mb-1 block">Date de naissance</label>
                        <input type="date" value={traveler.dateOfBirth} onChange={(e) => updateTravelerDetail(index, 'dateOfBirth', e.target.value)}
                          style={{ backgroundColor: t.bgBadge, borderColor: t.border, color: t.text }}
                      ...
                      <div>
                        ...
                        <input type="text" value={traveler.nationality} onChange={(e) => updateTravelerDetail(index, 'nationality', e.target.value)}
                          style={{ backgroundColor: t.bgBadge, borderColor: t.border, color: t.text }}
                          className="w-full border rounded-full px-4 py-2 text-sm focus:border-[#2d7a5a] outline-none" placeholder="Nationalité" />
                      </div>
                      <div className="col-span-2">
                        <label style={{ color: t.textMuted }} className="text-xs mb-1 block">Numéro de passeport (optionnel)</label>
                        <input type="text" value={traveler.passportNumber} onChange={(e) => updateTravelerDetail(index, 'passportNumber', e.target.value)}
                          style={{ backgroundColor: t.bgBadge, borderColor: t.border, color: t.text }}
                          className="w-full border rounded-full px-4 py-2 text-sm focus:border-[#2d7a5a] outline-none" placeholder="Numéro de passeport" />
                      </div>
                      {index === 0 && (
                        <>
                          <div>
                            <label style={{ color: t.textMuted }} className="text-xs mb-1 block">Email *</label>
                            <input type="email" value={traveler.email} onChange={(e) => updateTravelerDetail(index, 'email', e.target.value)}
                              style={{ backgroundColor: t.bgBadge, borderColor: t.border, color: t.text }}
                              className="w-full border rounded-full px-4 py-2 text-sm focus:border-[#2d7a5a] outline-none" placeholder="Email" />
                          </div>
                          <div>
                            <label style={{ color: t.textMuted }} className="text-xs mb-1 block">Téléphone *</label>
                            <input type="tel" value={traveler.phone} onChange={(e) => updateTravelerDetail(index, 'phone', e.target.value)}
                              style={{ backgroundColor: t.bgBadge, borderColor: t.border, color: t.text }}
                              className="w-full border rounded-full px-4 py-2 text-sm focus:border-[#2d7a5a] outline-none" placeholder="Téléphone" />
                          </div>
                        </>
                      )}
                    </div>
              ...
              <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div style={{ borderColor: t.border, background: `linear-gradient(to right, ${t.primary}1a, transparent)` }} className="border rounded-3xl p-6">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between"><span style={{ color: t.textMuted }}>Destination</span><span style={{ color: t.text }}>{transport?.destination?.name}</span></div>
                    <div className="flex justify-between"><span style={{ color: t.textMuted }}>Vol</span><span style={{ color: t.text }}>{transport?.name}</span></div>
                    <div className="flex justify-between"><span style={{ color: t.textMuted }}>Trajet</span><span style={{ color: t.text }}>{tripType === 'round_trip' ? 'Aller-retour' : 'Aller simple'}</span></div>
                    <div className="flex justify-between"><span style={{ color: t.textMuted }}>Voyageurs</span><span style={{ color: t.text }}>{totalTravelers}</span></div>
                    <div style={{ borderColor: t.border }} className="flex justify-between pt-2 border-t">
                      <span style={{ color: t.textMuted }}>Prix unitaire</span><span style={{ color: t.text }}>{parseFloat(transport?.price || 0).toFixed(2)} DA</span>
                    </div>
                    <div className="flex justify-between text-lg font-bold">
                      <span style={{ color: t.primary }}>Total</span><span style={{ color: t.primary }}>{totalPrice.toFixed(2)} DA</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {[
                      { id: 'chargily', label: 'Carte Edahabia / CIB', Icon: Smartphone },
                      { id: 'stripe', label: 'Carte Bancaire', Icon: CreditCard },
                      { id: 'on_arrival', label: "Payer à l'arrivée", Icon: Banknote },
                    ].map(({ id, label, Icon }) => (
                      <button key={id} onClick={() => setPaymentMethod(id)}
                        className="p-4 rounded-3xl font-medium transition border-2 flex flex-col items-center gap-2"
                        style={{
                          borderColor: paymentMethod === id ? t.primary : t.border,
                          backgroundColor: paymentMethod === id ? `${t.primary}1a` : t.bgBadge,
                          color: paymentMethod === id ? t.primary : t.textMuted
                        }}>
                ...
                {paymentMethod === 'chargily' && (
                  <div style={{ backgroundColor: isDark ? '#2d7a5a0d' : '#f7f5f080', borderColor: t.border }} className="border rounded-full px-6 py-4">
                    <p style={{ color: t.textMuted }} className="text-sm">Vous serez redirigé vers la page de paiement sécurisée de Chargily pour payer avec votre carte Edahabia ou CIB.</p>
                  </div>
                )}
                {paymentMethod === 'stripe' && (
                  <div style={{ backgroundColor: isDark ? '#2d7a5a0d' : '#f7f5f080', borderColor: t.border }} className="border rounded-full px-6 py-4">
                    <p style={{ color: t.textMuted }} className="text-sm">Vous serez redirigé vers la page de paiement sécurisée de Stripe pour payer avec votre carte bancaire internationale.</p>
                    <div style={{ color: t.textMuted }} className="mt-3 flex items-center gap-2 text-xs">
                      <CheckCircle className="w-4 h-4" /><span>Paiement 100% sécurisé par Stripe</span>
                    </div>
                  </div>
                )}
                {paymentMethod === 'on_arrival' && (
                  <div style={{ backgroundColor: isDark ? '#2d7a5a0d' : '#f7f5f080', borderColor: t.border }} className="border rounded-full px-6 py-4">
                    <p style={{ color: t.textMuted }} className="text-sm">Votre réservation sera enregistrée avec le statut "En attente de paiement". Vous paierez directement sur place.</p>
                  </div>
                )}
        ...
        {step < 4 && (
          <div style={{ borderColor: t.border }} className="flex items-center justify-between p-6 border-t">
            {step > 1 ? (
              <button onClick={handlePrevStep} style={{ color: t.textMuted }} className="flex items-center gap-2 hover:opacity-70 transition px-4 py-2">
                <ChevronLeft className="w-5 h-5" /> Retour
              </button>
            ) : <div />}
            {step < 3 && (
              <button onClick={handleNextStep} className="bg-[#2d7a5a] text-black font-bold px-8 py-3 rounded-full hover:bg-[#1a4a36] transition flex items-center gap-2">
