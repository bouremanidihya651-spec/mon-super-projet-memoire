...
import { ArrowLeft, Star, MapPin, Wifi, Coffee, Car, Heart } from 'lucide-react';
import { motion } from 'framer-motion';
import ReviewSection from '../components/ReviewSection';

const HotelDetail = ({ openAuthModal, isChatbotOpen, toggleChatbot }) => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchHotel = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/hotels/${id}`);
    ...
    return (
      <div className="min-h-screen bg-[#f7f5f0] dark:bg-dark-bg text-[#1a4a36] dark:text-dark-text flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-3xl font-serif italic mb-4">Hôtel non trouvé</h1>
          <Link to="/hotels">
  ...
  return (
    <div className="min-h-screen bg-[#f7f5f0] dark:bg-dark-bg text-[#1a4a36] dark:text-dark-text">
      ...
      <div className="relative h-96 overflow-hidden">
        <img
          src={hotel.image_url || "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=1920&q=80"}
          alt={hotel.name}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-12">
          <div className="max-w-7xl mx-auto">
            ...
            >
              ...
              <h1 className="text-5xl md:text-6xl font-serif italic mb-4 text-white">{hotel.name}</h1>
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-6 h-6 ${i < (hotel.stars || 5) ? 'text-[#c9a844] fill-current' : 'text-white/30'}`}
                    />
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  <Star className="w-5 h-5 text-[#c9a844] fill-current" />
                  <span className="text-xl text-white font-bold">{hotel.rating}</span>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
            ...
            >
              <h2 className="text-3xl font-serif italic mb-6 text-[#1a4a36] dark:text-dark-text">À propos de cet hôtel</h2>
              <p className="text-[#2d7a5a] dark:text-surface text-lg leading-relaxed mb-8">{hotel.description}</p>

              {/* Amenities */}
              <div className="bg-white dark:bg-dark-surface rounded-2xl p-6 border border-[#e0dcd4] dark:border-dark-border shadow-sm">
                <h3 className="text-xl font-serif font-bold mb-4 text-[#1a4a36] dark:text-dark-text">Équipements</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {hotel.amenities?.includes('wifi') && (
                    <div className="flex items-center gap-2">
                      <Wifi className="w-5 h-5 text-[#2d7a5a] dark:text-surface" />
                      <span className="text-[#2d7a5a] dark:text-surface">WiFi haut débit</span>
                    </div>
                  )}
                  {hotel.amenities?.includes('spa') && (
                    <div className="flex items-center gap-2">
                      <span className="text-[#2d7a5a] dark:text-surface">Spa & Bien-être</span>
                    </div>
                  )}
                  {hotel.amenities?.includes('restaurant') && (
                    <div className="flex items-center gap-2">
                      <Coffee className="w-5 h-5 text-[#2d7a5a] dark:text-surface" />
                      <span className="text-[#2d7a5a] dark:text-surface">Restaurant gastronomique</span>
                    </div>
                  )}
                  {hotel.amenities?.includes('piscine') && (
                    <div className="flex items-center gap-2">
                      <span className="text-[#2d7a5a] dark:text-surface">Piscine</span>
                    </div>
                  )}
                  {hotel.amenities?.includes('plage') && (
                    <div className="flex items-center gap-2">
                      <span className="text-[#2d7a5a] dark:text-surface">Accès plage</span>
                    </div>
                  )}
                  {hotel.amenities?.includes('fitness') && (
                    <div className="flex items-center gap-2">
                      <span className="text-[#2d7a5a] dark:text-surface">Centre de fitness</span>
                    </div>
                  )}
                  {hotel.amenities?.includes('bar') && (
                    <div className="flex items-center gap-2">
                      <span className="text-[#2d7a5a] dark:text-surface">Bar</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="lg:col-span-1">
            ...
            >
              <h3 className="text-2xl font-serif italic mb-6 text-[#1a4a36] dark:text-dark-text">Réserver cette chambre</h3>

              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-[#6b8f7b] dark:text-dark-text-muted">Prix par nuit</span>
                  <span className="text-3xl font-bold text-[#1a4a36] dark:text-dark-text">{hotel.price}DA</span>
                </div>
              </div>
        ...
        {/* --- SCORES SECTION --- */}
        {(hotel.luxury_score || hotel.comfort_score || hotel.service_score ||
          hotel.location_score || hotel.amenities_score) && (
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView="animate"
            ...
            className="mt-16"
          >
            <h2 className="text-3xl font-serif italic mb-8 text-center text-[#1a4a36] dark:text-dark-text">Scores de l'hôtel</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
              {hotel.luxury_score && (
                <div className="bg-white dark:bg-dark-surface rounded-xl p-4 border border-[#e0dcd4] dark:border-dark-border text-center shadow-sm">
                  <div className="text-[#2d7a5a] dark:text-surface text-2xl font-bold mb-2">{(hotel.luxury_score * 10).toFixed(0)}/10</div>
                  <div className="text-[#6b8f7b] dark:text-dark-text-muted text-sm">Luxe</div>
                </div>
              )}
              {hotel.comfort_score && (
                <div className="bg-white dark:bg-dark-surface rounded-xl p-4 border border-[#e0dcd4] dark:border-dark-border text-center shadow-sm">
                  <div className="text-[#2d7a5a] dark:text-surface text-2xl font-bold mb-2">{(hotel.comfort_score * 10).toFixed(0)}/10</div>
                  <div className="text-[#6b8f7b] dark:text-dark-text-muted text-sm">Confort</div>
                </div>
              )}
              {hotel.service_score && (
                <div className="bg-white dark:bg-dark-surface rounded-xl p-4 border border-[#e0dcd4] dark:border-dark-border text-center shadow-sm">
                  <div className="text-[#2d7a5a] dark:text-surface text-2xl font-bold mb-2">{(hotel.service_score * 10).toFixed(0)}/10</div>
                  <div className="text-[#6b8f7b] dark:text-dark-text-muted text-sm">Service</div>
                </div>
              )}
              {hotel.location_score && (
                <div className="bg-white dark:bg-dark-surface rounded-xl p-4 border border-[#e0dcd4] dark:border-dark-border text-center shadow-sm">
                  <div className="text-[#2d7a5a] dark:text-surface text-2xl font-bold mb-2">{(hotel.location_score * 10).toFixed(0)}/10</div>
                  <div className="text-[#6b8f7b] dark:text-dark-text-muted text-sm">Emplacement</div>
                </div>
              )}
              {hotel.amenities_score && (
                <div className="bg-white dark:bg-dark-surface rounded-xl p-4 border border-[#e0dcd4] dark:border-dark-border text-center shadow-sm">
                  <div className="text-[#2d7a5a] dark:text-surface text-2xl font-bold mb-2">{(hotel.amenities_score * 10).toFixed(0)}/10</div>
                  <div className="text-[#6b8f7b] dark:text-dark-text-muted text-sm">Équipements</div>
                </div>
              )}
            </div>
          </motion.div>
        ...
        {/* Reviews Section */}
        <ReviewSection targetType="hotel" targetId={id} openAuthModal={openAuthModal} />
      </div>
