...
router.post('/create-stripe-checkout', authenticateToken, async (req, res) => {
  try {
    ...
    const { amount, currency = 'dzd', reservationId } = req.body;

    if (!amount || amount <= 0) {
      return res.status(400).json({ error: 'Invalid amount' });
    }

    // Convert DZD to EUR for Stripe (fixed rate: 1 EUR = 145 DZD)
    const exchangeRate = 145;
    const amountInEUR = amount / exchangeRate;
    const amountInEURCentimes = Math.round(amountInEUR * 100);

    console.log('Creating Stripe session with:', {
      originalAmountDZD: amount,
      convertedAmountEUR: amountInEUR.toFixed(2),
      amountCentimes: amountInEURCentimes,
      reservationId
    });
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `Réservation #${reservationId} (${amount} DA)`,
            },
            unit_amount: amountInEURCentimes, // Amount in EUR cents
          },
          quantity: 1,
        },
      ],
...
router.post('/create-payment-intent', authenticateToken, async (req, res) => {
  try {
    ...
    // Convert DZD to EUR for Stripe (fixed rate: 1 EUR = 145 DZD)
    const exchangeRate = 145;
    const amountInEURCentimes = Math.round((amount / exchangeRate) * 100);

    // Create Payment Intent in TEST mode
    const paymentIntent = await stripe.paymentIntents.create({
      amount: amountInEURCentimes,
      currency: 'eur',
      metadata: {
        reservationId: reservationId?.toString() || '',
        userId: req.user.id.toString(),
        amountDZD: amount.toString()
      },
...
router.post('/create-chargily-checkout', authenticateToken, async (req, res) => {
  try {
    const { amount, currency = 'dzd', reservationId, customer } = req.body;

    if (!amount || amount <= 0) {
      return res.status(400).json({ error: 'Invalid amount' });
    }

    // Validate amount is reasonable (max 2,000,000 DZD for safety)
    if (amount > 2000000) {
    ...
    // Amount is already in DZD, Chargily expects amount in whole DZD
    const amountInDZD = parseFloat(amount);
