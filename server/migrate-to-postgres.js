      ...
      for (const row of rows) {
        try {
          const values = columns.map(col => {
            ...
            // Gérer les booléens SQLite (0/1) → PostgreSQL (true/false)
            if (typeof val === 'number' && (val === 0 || val === 1)) {
              if (/^(is|has|active|enabled|verified|published|emailVerified|isAdmin)/i.test(col)) {
                return val === 1;
              }
            }
            ...
            // Gérer les NULL et valeurs vides
