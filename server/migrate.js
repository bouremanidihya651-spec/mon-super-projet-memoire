...
db.serialize(() => {
  ...
  db.all("PRAGMA table_info(Users)", (err, rows) => {
    ...
    function checkIsBlocked() {
      // Ajouter isBlocked si manquant
      if (!columnNames.includes('isBlocked')) {
        console.log('➕ Ajout de la colonne isBlocked à Users...');
        db.run("ALTER TABLE Users ADD COLUMN isBlocked INTEGER DEFAULT 0", (err) => {
          if (err) {
            console.error('Erreur ajout isBlocked:', err);
          } else {
            console.log('✅ Colonne isBlocked ajoutée');
          }
          checkProfilePhoto();
        });
      } else {
        console.log('ℹ️  Colonne isBlocked déjà existante');
        checkProfilePhoto();
      }
    }
